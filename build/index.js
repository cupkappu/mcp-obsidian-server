...
class ObsidianMCPServer {
    ...
    getConfig() {
        ...
        try {
            ...
            throw new McpError(ErrorCode.InvalidParams, `Invalid configuration: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ensureClient() {
        if (!this.client) {
            ...
            try {
                switch (name) {
                    ...
                    case 'execute_command': {
                        if (!args?.commandId)
                            throw new McpError(ErrorCode.InvalidParams, 'Command ID is required');
                    ...
                    case 'get_periodic_note': {
                        if (!args?.period)
                            throw new McpError(ErrorCode.InvalidParams, 'Period is required');
                        const content = await client.getPeriodicNote(args.period, args?.asJson || false);
                        return {
                            content: [
                                {
                                    type: 'text',
